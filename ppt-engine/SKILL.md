...
name: ppt-engine
description: >
  Internal SVG-to-PPTX backend engine for ppt-builder. Provides project management, SVG
  finalization/checking, and SVG-to-PPTX export tooling.普通用户走 ppt-builder;本技能
  面向直接调用脚本或排障。Do not trigger directly for building a presentation deck.
upstream: hugohe3/ppt-master (MIT License, copyright 2025-2026 Hugo He)
---

# PPT Engine Skill

> 内部后端引擎,为 `ppt-builder` 提供 SVG 定稿与 PPTX 导出能力。普通用户请触发 `ppt-builder`,本技能面向脚本直调与排障。
>
> Upstream: <https://github.com/hugohe3/ppt-master.git> — MIT License, copyright 2025-2026 Hugo He. 本目录由上游本地 vendor 而来,仅重命名目录与 frontmatter,未改动其代码逻辑。
